...
pub fn probe_rar_local(path: &Path) -> Option<(Option<Vec<u8>>, Option<Vec<u8>>)> {
    ...
    let param = unsafe {
        if !param_ptr.is_null() && param_size > 0 {
            let slice = std::slice::from_raw_parts(param_ptr as *const u8, param_size as usize);
    ...
    let cover = unsafe {
        if !cover_ptr.is_null() && cover_size > 0 {
            let slice = std::slice::from_raw_parts(cover_ptr as *const u8, cover_size as usize);
            let vec = slice.to_vec();
            libc::free(cover_ptr as *mut libc::c_void);
            Some(vec)
        } else {
