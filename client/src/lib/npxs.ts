 ...
 *
 * We isolate the detection here so the rule lives in one place:
 *
 ...
 *
 * (A second consumer — the former standalone install queue — used the
 * same regex for its fire-and-forget NPXS path; that queue was removed
 * once installs moved to the per-console pkgLibrary store.) Centralising
 * the regex (a) eliminates drift, (b) makes the rule unit-testable in
 * isolation, and (c) keeps one canonical comment about *why* NPXS pkgs
 * are special.
 */
